...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Management Mata Kuliah</h3>
    </div>
    <div class="card-body">
    ...
    <div class="col-12">
        <div class="card card-outline card-primary">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="{{ route('matkul.create') }}" class="btn btn-primary bg-info btn-tool" id="btn-create">Tambah
                        Mata Kuliah</a>
                </div>
            </div>
            <section id="loading">
                    ...
                    <table id="dt" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="x-grid-cell-inner">{{ $value->nama_matkul ?? '-' }}</td>
                                <td class="x-grid-cell-inner">{{ $value->sks ?? '-' }}</td>
                                <td class="x-grid-cell-inner">{{ $value->semester ?? '-' }}</td>
                                <td class="x-grid-cell-inner">{{ $value->jurusan->nama_jurusan ?? '-' }}</td>
                                <td class="x-grid-cell-inner">
                                    <div class="row">
                                        <a href="{{ route('matkul.edit', $value->id) }}" class="btn btn-sm btn-outline-warning mx-2" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></a>
                                        <form method="post" onsubmit="return confirm('Hapus Data atas nama {{ $value->nama_matkul }}?');" action="{{ route('matkul.delete', $value->id) }}">
                                            @csrf
                                            @method('delete')
